...
// Returns the cryptographic signature type associated with the RSA signer, indicating SHA-256 with RSA is used for signing.
func (s *rsaSigner) Type() ndn.SigType {
	return ndn.SignatureSha256WithRsa
}

// Returns the name of the RSA key associated with this signer.
func (s *rsaSigner) KeyName() enc.Name {
	return s.name
}

// Returns the name used as the key locator for the RSA signing instance.
func (s *rsaSigner) KeyLocator() enc.Name {
	return s.name
}

// Returns the size of the RSA key in bytes, which serves as an estimate for the size of the signature generated by this signer.
func (s *rsaSigner) EstimateSize() uint {
	return uint(s.key.Size())
}

// Signs the concatenated content of the provided buffers using RSA PKCS1v15 with SHA-256, returning the digital signature and any error encountered.
...
// Returns the RSA public key in PKIX-encoded byte format for verification by other nodes.
func (s *rsaSigner) Public() ([]byte, error) {
	return x509.MarshalPKIXPublicKey(&s.key.PublicKey)
}

// Returns the RSA private key in PKCS#1 format as a byte slice.
